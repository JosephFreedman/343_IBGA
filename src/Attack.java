...
public class Attack {
    ...
    public void attackToControl(Group attacker, Group attacked, int money) {
        ...
        // If target card is a Group you already control, it cannot be attacked
        // Attacking Group must have an outward arrow free
        // Aiding Attacks: If a Group does not attack during a turn, it may use its transferable power to aidan attack made by any other Group in its Power Structure. If a Group has two Power numbers separated by a slash (like 5/2), the second number is its transferable power. The UFO's can attack twice per turn. Example: A Group with a power of 6, aided by another Group with a transferable power of 4, is attacking a Group with a resistance of 3. The attacker has a total power of 10 (6 plus 4), and will have to roll a 7 or less (10 minus 3) to succeed.
        // Power Structure Position. If a Group is already controlled by an opponent, it may be harder to control. The closer it is to the Illuminati who control it, the more of a bonus it gets to Resistance (or to Power if someone tries to destroy it). If it is adjacent, it gets a +10! If it is one Group away, it gets a +5. If it is two Groups away, it gets a +2
        ...
        // Special Powers
        // Spending Money to Attack
        attackerPowerBonus += money;

        // To find the roll required for a successful attack, subtract the defending Group’s Resistance from the attacking Group’s  Power. Example: If a Power of 6 attacks a Resistance of 2, it can succeed only on a roll of 4 or less
